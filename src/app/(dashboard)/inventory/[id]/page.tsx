...
export default async function SessionDetailsPage({ params }: SessionDetailsPageProps) {
  ...
  // Check if they are the assigned Inventory Person
  const inventoryPersonProfile = await prisma.inventoryPerson.findFirst({
    where: { user: { id: session.user.id }, isActive: true }
  });
  const isAssignedPerson = inventoryPersonProfile && sDetails.inventoryPersonId === inventoryPersonProfile.id;

  if (!isPAOOrAdmin && !isAssignedPerson) {
    return (
      <div className="p-8 text-center text-red-655 font-bold">
        Permission denied. You are not assigned to perform inventory counting for this session.
      </div>
    );
  }
